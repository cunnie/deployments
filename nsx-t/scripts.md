Following are the commands we used when upgrading our PCF foundation.
...
curl "$FOUNDATION_URL/api/v0/staged/products/$CF_GUID/jobs" -H "Authorization: Bearer $UAA_ACCESS_TOKEN" -k | jq -r '.jobs[] | select(.name=="router" or .name=="diego_brain" or .name=="tcp_router")'
  # Get the Current Resource Configs of the VMs in the pools
set -- \
...
while [ $# -gt 0 ]; do
 ...
 # Here we upgrade Operations Manager to 2.3.0, and upload and stage (but not deploy)
 # the NSX-T 2.3 tile

 # Set the VM extensions from the files we've already created
for VM_EXTENSION_FILE in {router,diego_brain,tcp_router}_vm-extension.json; do
 ...
 # Here we upload and stage (but not deploy) the PAS 2.3 tile

 # Set the new Resource Configs which refer to the VM Extensions
set -- \
